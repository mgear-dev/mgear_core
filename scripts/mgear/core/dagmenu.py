...
def get_mgear_file_drop_path():
    """Get the mGear mGearFileDropAction.mel override path

    Returns:
        str: path to file
    """
    mel_dir = os.path.abspath(os.path.dirname(__file__))
    file_drop_fp = os.path.join(mel_dir, "mGearFileDropAction.mel")
    return os.path.abspath(file_drop_fp).replace("\\", "/")
...
def get_original_file_drop_action():
    ...
    orig_mel_file = orig_mel_file_str.replace(mel_string_start, "")
    if orig_mel_file_str.startswith(mel_string_start) and orig_mel_file != get_mgear_file_drop_path():
...
def mgear_file_drop_toggle(new_state):
    ...
    mgear_file_drop_path = get_mgear_file_drop_path()
    if new_state:
        mel_cmd = 'source "{}";'.format(mgear_file_drop_path)
    else:
...
def guide_file_prompt(guide_filePath):
    ...
    if results == "Import":
        shifter.io.import_guide_template(filePath=guide_filePath)
    elif results == "Build":
        shifter.io.build_from_file(filePath=guide_filePath)
    else:
        pass
...
def install_utils_menu(m):
    ...
    cmds.menuItem("mgear_file_drop_menuitem", label="Enable mGear file drop",
                  command=mgear_file_drop_toggle, checkBox=state)
    cmds.menuItem(divider=True)
